...
feaFile = '../data_VIPeR/';
list_a = dir([feaFile,'*cam_a*']);
list_b = dir([feaFile,'*cam_b*']);
pcaDims = -1;
numClass = 632;
numFolds = 10;
numRanks = 100;          
...
for i=1: numClass
list_fea{i} = list_a(i).name;      

fea_a{i}= strcat([feaFile,list_fea{i}]);   
load(fea_a{1,i});

descriptors_a= [descriptors_a;tmp_fea'];         
end

for i=1: numClass
list_fea{i} = list_b(i).name;      

fea_b{i}= strcat([feaFile,list_fea{i}]);   
load(fea_b{1,i});

descriptors_b= [descriptors_b;tmp_fea'];         
end
%% load the extracted LOMO features
galFea = descriptors_a(1 : numClass, :); 
probFea = descriptors_b(1 : numClass, :); 
clear descriptors_a descriptors_b
...
for nf = 1 : numFolds
    ...
    galFea1 = galFea( p(1:numClass/2), : ); % 1 n?a ?nh ? t?p gallery
    probFea1 = probFea( p(1:numClass/2), : );  % n?a còn l?i ? probe
...
meanCms = meanCms'*100;
csvwrite('../results_VIPeR/result_XQDA.csv',meanCms);